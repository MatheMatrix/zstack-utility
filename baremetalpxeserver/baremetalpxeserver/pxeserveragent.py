...
class PxeServerAgent(object):
    ...
    def _start_pxe_server(self):
        ret, _, err = bash_roe(
            "ps -ef | grep -v 'grep' | grep 'dnsmasq -C {0}' || dnsmasq -C {0} -u root".format(self.DNSMASQ_CONF_PATH))
        if ret != 0:
            raise PxeServerError("failed to start dnsmasq on baremetal pxeserver[uuid:%s]: %s" % (self.uuid, err))

        ret, _, err = bash_roe(
            "ps -ef | grep -v 'grep' | grep 'vsftpd {0}' || vsftpd {0}".format(self.VSFTPD_CONF_PATH))
        if ret != 0:
...
"""
        ...
        if not os.path.exists(self.NOVNC_INSTALL_PATH):
            ret = bash_r(
                "tar -xf %s -C %s" % (os.path.join(self.BAREMETAL_LIB_PATH, "noVNC.tar.gz"), self.BAREMETAL_LIB_PATH))
            if ret != 0:
    ...
    def create_bm_configs(self, req):
        ...
        logger.info(
            "successfully created pxelinux.cfg and preconfiguration file for baremetal instance[uuid:%s] on pxeserver[uuid:%s]" % (
            cmd.bmUuid, self.uuid))
        return json_object.dumps(rsp)
    ...
    def _create_pxelinux_cfg(self, cmd):
        ...
        if cmd.preconfigurationType == 'kickstart':
            ...
            append = 'ip=dhcp url=ftp://{PXESERVER_DHCP_NIC_IP}/{IMAGEUUID} autoinstall ds=nocloud-net\;s=http://{PXESERVER_DHCP_NIC_IP}:7773/ks/{KS_CFG_NAME}/ ---'
...
"""
    ...
    def delete_bm_configs(self, req):
        ...
        logger.info(
            "successfully deleted pxelinux.cfg and ks.cfg %s" % cmd.pxeNicMac if cmd.pxeNicMac != '*' else 'all')
        return json_object.dumps(rsp)
    ...
    def create_bm_nginx_proxy(self, req):
        ...
        logger.info("successfully create terminal nginx proxy for baremetal instance[uuid:%s] on pxeserver[uuid:%s]" % (
        cmd.bmUuid, self.uuid))
        return json_object.dumps(rsp)
    ...
    def delete_bm_nginx_proxy(self, req):
        ...
        logger.info(
            "successfully deleted terminal nginx proxy for baremetal instance[uuid:%s] on pxeserver[uuid:%s]" % (
            cmd.bmUuid, self.uuid))
        return json_object.dumps(rsp)
    ...
    def create_bm_novnc_proxy(self, req):
        ...
        logger.info("successfully created novnc proxy for baremetal instance[uuid:%s] on pxeserver[uuid:%s]" % (
        cmd.bmUuid, self.uuid))
        return json_object.dumps(rsp)
    ...
    def delete_bm_novnc_proxy(self, req):
        ...
        logger.info("successfully deleted novnc proxy for baremetal instance[uuid:%s] on pxeserver[uuid:%s]" % (
        cmd.bmUuid, self.uuid))
        return json_object.dumps(rsp)
    ...
    def create_bm_dhcp_config(self, req):
        ...
        logger.info("successfully created dhcp config for baremetal chassis[uuid:%s] on pxeserver[uuid:%s]" % (
        cmd.chassisUuid, self.uuid))
        return json_object.dumps(rsp)
    ...
    def delete_bm_dhcp_config(self, req):
        ...
        logger.info("successfully deleted dhcp config for baremetal chassis[uuid:%s] on pxeserver[uuid:%s]" % (
        cmd.chassisUuid, self.uuid))
        return json_object.dumps(rsp)
    ...
    def download_imagestore(self, req):
        ...
        if not rsp.success:
            raise PxeServerError(
                "failed to download image[uuid:%s] from imagestore to baremetal image cache" % cmd.imageUuid)
        ...
        if ret != 0:
            raise PxeServerError(
                "failed to mount image[uuid:%s] to baremetal ftp server %s" % (cmd.imageUuid, cache_path))
        ...
        ret1 = bash_r("cp %s %s && chmod 777 %s" % (
        os.path.join(mount_path, "isolinux/vmlinuz*"), vmlinuz_file_path, vmlinuz_file_path))
        ret2 = bash_r("cp %s %s && chmod 777 %s" % (
        os.path.join(mount_path, "isolinux/initrd*.img"), initrd_file_path, initrd_file_path))
        # DEBIAN SERVER
        ret3 = bash_r("cp %s %s && chmod 777 %s" % (
        os.path.join(mount_path, "install/netboot/*-installer/amd64/linux"), vmlinuz_file_path, vmlinuz_file_path))
        ret4 = bash_r("cp %s %s && chmod 777 %s" % (
        os.path.join(mount_path, "install/netboot/*-installer/amd64/initrd.gz"), initrd_file_path, initrd_file_path))
        # SUSE
        ret5 = bash_r("cp %s %s && chmod 777 %s" % (
        os.path.join(mount_path, "boot/*/loader/linux"), vmlinuz_file_path, vmlinuz_file_path))
        ret6 = bash_r("cp %s %s && chmod 777 %s" % (
        os.path.join(mount_path, "boot/*/loader/initrd"), initrd_file_path, initrd_file_path))
        # ns10
        ret7 = bash_r("cp %s %s && chmod 777 %s" % (
        os.path.join(mount_path, "images/pxeboot/vmlinuz"), vmlinuz_file_path, vmlinuz_file_path))
        ret8 = bash_r("cp %s %s && chmod 777 %s" % (
        os.path.join(mount_path, "images/pxeboot/initrd.img"), initrd_file_path, initrd_file_path))
        # ubuntu live-server
        ret9 = bash_r("cp %s %s && chmod 777 %s" % (
        os.path.join(mount_path, "casper/vmlinuz"), vmlinuz_file_path, vmlinuz_file_path))
        ret10 = bash_r("cp %s %s && chmod 777 %s" % (
        os.path.join(mount_path, "casper/initrd"), initrd_file_path, initrd_file_path))
        if (ret1 != 0 or ret2 != 0) and (ret3 != 0 or ret4 != 0) and (ret5 != 0 or ret6 != 0) and (
                ret7 != 0 or ret8 != 0) and (ret9 != 0 or ret10 != 0):
            raise PxeServerError(
                "failed to copy vmlinuz and initrd.img from image[uuid:%s] to baremetal tftp server" % cmd.imageUuid)
