...
def test_remove_host_cache_lvm_filter_devices_keeps_sharedblock_devices(lvm_configs):
    ...
    vcp.append_host_cache_lvm_filter_devices(["/dev/cache-a", "/dev/cache-b"], config_files=config_files)
    vcp.remove_host_cache_lvm_filter_devices(["/dev/cache-a"], config_files=config_files)
    ...
    for path in lvm_configs:
        for key in vcp.LVM_FILTER_KEYS:
            ...
            assert "a|^/dev/sharedblock-a$|" in rules
            assert "a|^/dev/sharedblock-b$|" in rules
            assert "a|^/dev/cache-a$|" not in rules
            assert "a|^/dev/cache-b$|" in rules
            assert rules[-1] == "r|.*|"
