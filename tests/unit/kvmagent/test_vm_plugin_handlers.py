...
class TestCancelVolumeCbtBackupHandler:
    ...
    def test_check_cbt_capabilities(self):
        plugin = _make_vm_plugin()
        client = MagicMock()
        client.get_cbt_capabilities.return_value = {'rollbackVolumeCbtBackup': True}
        vm_plugin.ImageStoreClient = MagicMock(return_value=client)

        result = plugin.check_cbt_capabilities(_make_req({}))
        rsp = json.loads(result)

        assert rsp['success'] is True
        assert rsp['rollbackVolumeCbtBackup'] is True
        client.get_cbt_capabilities.assert_called_once_with()

    def test_check_cbt_capabilities_does_not_advertise_unsupported_cli(self):
        plugin = _make_vm_plugin()
        client = MagicMock()
        client.get_cbt_capabilities.return_value = {}
        vm_plugin.ImageStoreClient = MagicMock(return_value=client)
        ...
        assert rsp['success'] is True
        assert rsp['rollbackVolumeCbtBackup'] is False

    def test_check_cbt_capabilities_rejects_legacy_cli_command_failure(self):
        plugin = _make_vm_plugin()
        client = MagicMock()
        client.get_cbt_capabilities.side_effect = Exception("'cbtcapabilities' is not a valid subcommand")
        vm_plugin.ImageStoreClient = MagicMock(return_value=client)
        ...
        assert 'not a valid subcommand' in rsp['error']
...
class TestExportNbdVolumesHandler:
    ...
    def test_export_nbd_volumes_uses_configured_cbd_path(self):
        ...
        assert rsp['success'] is True
        plugin.active_volume_if_need.assert_called_once_with(configured_path)
...
class TestGetVolumesCbtBitmapsHandler:
    ...
    def test_get_full_zbs_bitmap_unions_source_diff_and_scratch_qcow2(self):
        ...
        with patch.object(vm_plugin.jsonobject, 'loads', return_value=cmd), \
                patch.object(vm_plugin, 'is_zbs_install_path', return_value=True), \
                patch.object(vm_plugin, 'get_zbs_cli_path', return_value='logical/volume'), \
                patch.object(
                    vm_plugin,
                    'get_cbd_actual_path',
                    return_value='cbd:physical/logical/scratch_zbs_:/etc/zbs/client.conf',
                ), \
                patch.object(vm_plugin.qemu, 'get_zbs_data_bitmap', return_value={0: 65536}) as get_zbs, \
        ...
        assert rsp['success'] is True
        get_zbs.assert_called_once_with('logical/volume', vm_plugin.MAX_NBD_READ_SIZE)
    ...
    def test_get_full_zbs_bitmap_propagates_clone_diff_business_error(self):
        ...
        with patch.object(vm_plugin.jsonobject, 'loads', return_value=cmd), \
                patch.object(vm_plugin, 'is_zbs_install_path', return_value=True), \
                patch.object(vm_plugin, 'get_zbs_cli_path', return_value='logical/clone-root'), \
                patch.object(
                    vm_plugin.qemu,
                    'get_zbs_data_bitmap',
...
class TestVmDiskHelpers:
    def test_cbt_cbd_path_appends_client_config_once(self):
        plugin = _make_vm_plugin()
        ...
        assert plugin.get_cbt_volume_actual_install_path(configured_path) == configured_path
        assert vm_plugin.make_cbd_conf(path) == configured_path[len(vm_plugin.PROTOCOL_CBD_PREFIX):]
        assert vm_plugin.make_cbd_conf(configured_path) == configured_path[len(vm_plugin.PROTOCOL_CBD_PREFIX):]
