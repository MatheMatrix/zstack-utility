...
ia_install_pip(){
    ...
    pypi_source="file://${ZSTACK_INSTALL_ROOT}/apache-tomcat/webapps/${app_name}/static/pypi/simple"
    if [ ! -z $DEBUG ]; then
...
check_zstack_server(){
    curl --noproxy -H "Content-Type: application/json" -d '{"org.zstack.header.apimediator.APIIsReadyToGoMsg": {}}' http://localhost:8080/${app_name}/api >>$ZSTACK_INSTALL_LOG 2>&1
    return $?
}
