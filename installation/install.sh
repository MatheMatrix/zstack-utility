...
grep "APP_USER" ~/.bashrc > /dev/null
if [ $? -eq 0 ]; then
    sed -i "s#export APP_USER=.*#export APP_USER=${app_name}#" ~/.bashrc
else
    echo "export APP_USER=${app_name}" >> ~/.bashrc
fi
echo "export APP_USER=${app_name}" > /etc/profile.d/app_name.sh
