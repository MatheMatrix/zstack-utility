...
remote_port = None
host_uuid = None
zbs_file_path = "/bin/zbs"
# common zbsprimarystorage deps of ky10 that need to update
...
if IS_AARCH64:
    ...
    qemu_img_local_pkg = "%s/qemu-img-aarch64" % zbsp_root
...
if host_info.distro in RPM_BASED_OS:
    ...
    if not qemu_installed:
        ...
        # c74 do not apply the change. Therefore, if zbs bs host is c74 and
...
# name: install python pkg and replace zbs python path
replace_content(zbs_file_path, "regexp='/usr/bin/env python' replace='/usr/bin/python2.7'", host_post_info)
extra_args = "\"--trusted-host %s -i %s \"" % (trusted_host, pip_url)
pip_install_arg = PipInstallArg()
pip_install_arg.extra_args = extra_args
pip_install_arg.name = "python-zbslibs"
...
if status is False:
    set_ini_file("/etc/zbs/zbs.conf", 'global', "rbd_default_format", "2", host_post_info)
