...
class ZbsAgent(plugin.TaskManager):
    ...
    def cbd_to_nbd(self, req):
        ...
        src_path = logical_pool_name + "/" + lun_name + "@" + snapshot_name
        dest_path = logical_pool_name + "/" + snapshot_name
        ...
        if '@' in cmd.installPath:
            ...
            if not isProtected:
                zbsutils.protect_snapshot(logical_pool_name, lun_name, snapshot_name)

            o = zbsutils.clone_volume(logical_pool_name, lun_name, snapshot_name, snapshot_name)
            ret = jsonobject.loads(o)
            if ret.error.code == 0:
                install_path = dest_path
            else:
                raise Exception(
                    'failed to clone lun[%s] to lun[%s], error[%s]' % (src_path, dest_path, ret.error.message))
        else:
