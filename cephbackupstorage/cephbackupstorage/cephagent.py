...
from zstacklib.utils import lock, upload_task
...
from zstacklib.utils.upload_task import StorageBackend, UploadParam, UploadTask, UploadHandler, StorageObject
...
class CephUploadTask(UploadTask):
    def __init__(self, imageUuid, dstPath, tmpPath, ioctx):
        super(CephUploadTask, self).__init__(imageUuid, dstPath)
    ...
    def complete_upload(self):
        try:
            ...
            self.fail('upload image %s failed: %s' % (self.imageUuid, str(e)))
            shell.run('rbd rm %s' % self.tmpPath)
            return

        if file_format == 'qcow2' and linux.qcow2_get_backing_file('rbd:' + self.tmpPath):
            self.fail('Qcow2 image %s has backing file' % self.imageUuid)
            shell.run('rbd rm %s' % self.tmpPath)
            return
        ...
        if file_format in ['qcow2', 'vmdk']:
            ...
            try:
                ...
                self.fail('cannot convert %s image %s to rbd' % (file_format, self.imageUuid))
                logger.warn('convert image %s failed: %s', (self.imageUuid, str(e)))
                return
            finally:
                shell.run('rbd rm %s' % self.tmpPath)
                if conf_path:
    ...
    def check_capacity(required_size):
        ...
        if avail <= required_size:
            return "Ceph capacity not enough for size: %d, available: %d" % (required_size, avail)
...
class CephAgent(object):
    ...
    def _prepare_upload(self, cmd):
        ...
        task = CephUploadTask(imageUuid, dstPath, tmpPath, self.get_ioctx(pool))
