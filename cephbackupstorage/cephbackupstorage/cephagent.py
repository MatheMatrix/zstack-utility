...
class CephAgent(object):
    ...
    COPY_FILES_PATH = "/ceph/files/copy"
    ...
    def __init__(self):
        ...
        self.http_server.register_async_uri(self.COPY_FILES_PATH, self.copy_file_to_host)
    ...
    def get_upload_file_progress(self, req):
        ...
        def get_unzip_file_size():
            unzipInstallPath = "%s_%s_%d" % (cmd.installPath, rsp.md5sum, int(time.time()))
            rsp.unzipInstallPath = unzipInstallPath

            shell.call('tar -zxf %s -C %s' % (task.installPath, unzipInstallPath))
    ...
    @replyerror
    def copy_file_to_host(self, req):
        cmd = jsonobject.loads(req[http.REQUEST_BODY])
        # scp -P 22222 -i /root/.ssh/id_rsa /root/cephagent.py root@192.168.1.100:/root/cephagent.py
        # tar -zxf /root/cephagent.py.tar.gz -C /root/
        # sshpass -p 'xxxx' ssh -p 50022 zmuser@127.0.0.1 "echo 'xxxx' | sudo -S tar -zxvf /tmp/GuestTools_linux-4.10.16.tar.gz -C /tmp"

        # sshpass -p 'xxxx' ssh -p 50022 zmuser@127.0.0.1 "echo 'xxxx' | sudo -S tar -zxvf /tmp/GuestTools_linux-4.10.16.tar.gz -C /tmp"

        # 执行
        # 解压 /tmp/GuestTools_linux-4.10.16.tar.gz
        # 获取脚本
