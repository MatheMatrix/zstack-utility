...
class ChangeIpCmd(Command):
    ...
    def update_morph_config(self, change_ip):
        """
        update morph config
        zstack-ctl start-extra-service --init --name morph
        """
        try:
            default_morph_path = "/var/lib/zstack/morph/"
            default_morph_config = default_morph_path + "application.yml"
            check_cmd = ShellCmd("systemctl list-units --type=service | grep -i morph", pipe=True)
            check_cmd(False)
...
ExecStart=/etc/alternatives/java_sdk_21/bin/java -jar {jar} --spring.config.location=file:{config} --spring.config.additional-location=file:{database_config}
...
WantedBy=multi-user.target
    '''.format(jar=default_morph_jar, config=default_morph_config, path=default_morph_path, database_config=default_morph_database_config)
