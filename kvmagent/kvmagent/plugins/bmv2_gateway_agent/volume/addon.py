...
class AddonVolume(base.BaseVolume):

    def __init__(self, *args, **kwargs):
        super(AddonVolume, self).__init__(*args, **kwargs)
...
# monIp: 172.27.16.181,172.27.16.99,...
    def prepare_instance_resource(self):
        ...
        uss_ips = sorted(mon_ip.split(','))
        for uss_ip in uss_ips:
            dev_name = linux.find_route_interface_by_destination_ip(uss_ip)
            shell.run("iptables -t nat -A PREROUTING -s %s -d %s -p tcp --dport 3260 -j DNAT --to-destination %s:3260" %
                      (self.instance_obj.provision_ip, instance_gateway_ip, uss_ip))
            shell.run("iptables -t nat -A POSTROUTING -s %s -o %s -j SNAT --to-source %s" % (
                self.instance_obj.provision_ip, dev_name, uss_ip))
