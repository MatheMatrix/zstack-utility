...
class Vm(object):
    ...
    def _detach_data_volume(self, volume):
        ...
        try:
            ...
            def delete_residual_block_node():
                if volume.deviceType == 'ceph':
                    ...
                    for node in residual_blockdevs:
                        ...
                        if r == 0:
                            ...
                            logger.debug("failed to delete vm[%s] residual block driver node[%s], because %s" % (self.uuid, node, str(err)))

            if not is_libvirt_support_blockdev(linux.get_libvirt_version()):
                delete_residual_block_node()
