...
<hostdev mode='subsystem' type='mdev' managed='yes' model='vfio-pci' display='off'>
    ...
    def _is_dgpu_shmem_attached(self, vm, shmem_name):
    ...
    def _wait_dgpu_shmem_attached_state(self, vm, shmem_name, attached, timeout=20):
        return linux.wait_callback_success(
            lambda _: self._is_dgpu_shmem_attached(vm, shmem_name) == attached,
            None,
            timeout=timeout,
            interval=1
        )
    ...
    def hot_plug_dgpu_shmem(self, req):
        ...
        try:
            ...
            if not self._wait_dgpu_shmem_attached_state(vm, shmem_name, True):
                rsp.success = False
    ...
    def hot_unplug_dgpu_shmem(self, req):
        ...
        try:
            ...
            if not self._is_dgpu_shmem_attached(vm, shmem_name):
            ...
            if not self._wait_dgpu_shmem_attached_state(vm, shmem_name, False):
                rsp.success = False
