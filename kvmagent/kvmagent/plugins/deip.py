...
class Eip(object):
    ...
    def apply_eip(self, eip):
        ...
        def set_gateway_arp_if_needed():
            ...
            create_ebtable_rule_if_needed('nat', CHAIN_NAME, "-p ARP --arp-op Request --arp-mac-src {{NIC_MAC_IN_EBTALES}} --arp-ip-src {{NIC_IP}} --arp-ip-dst {{NIC_GATEWAY}} -j arpreply --arpreply-mac {{GATEWAY_MAC}}")
        ...
        def add_filter_to_prevent_namespace_arp_request():
            ...
            create_ebtable_rule_if_needed('nat', PRI_ODEV_CHAIN, "-p ARP --arp-op Reply --arp-ip-dst {{NIC_IP}} -j ACCEPT")
            create_ebtable_rule_if_needed('nat', PRI_ODEV_CHAIN, "-p ARP --arp-op Reply -j DROP")
