...
# serializes hugepage read-modify-write and container lifecycle across
# concurrent activate/start/deactivate requests; RLock because ensure_target
# nests into ensure_free_hugepages.
...
def image_registry(image):
    # docker ref: [registry[:port]/]repo[:tag]. the first path segment is a
    # registry only if it carries a host marker ('.'/':'/localhost), otherwise
    # it's a repo namespace (e.g. "library/zbs-vhost").
    if "/" not in image:
        return None
...
def ensure_insecure_registry(registry):
    ...
    # SIGHUP reload applies insecure-registries without bouncing containers
    bash.bash_errorout("systemctl reload docker")


# markers docker emits when a registry speaks plain http or has an untrusted
# cert; only these justify downgrading it to insecure-registries.
...
def domain_memory_bytes(domain_xml):
    ...
    if mem is None or not (mem.text or "").strip():
        return 0
    # libvirt defaults the memory unit to KiB
...
def _compact_memory():
    ...
    # satisfy the contiguous 2MiB allocations guest RAM and the target need.
    bash.bash_o("sync; echo 3 > /proc/sys/vm/drop_caches; echo 1 > /proc/sys/vm/compact_memory")


@_locked
def ensure_free_hugepages(need_pages):
    # vhost-user-blk pins guest RAM to shared hugepages, so each vhost VM (and the
    # target container) needs need_pages FREE before qemu launches. running guests +
    # container already show up as (total - free); only grow when free is short.
...
def reclaim_hugepages(slack=0):
    # a stopped vhost VM leaves its pages free but nr_hugepages stays at the peak;
    # shrink back to what's in use (+slack) so non-vhost VMs can use the RAM. echo
    # only releases free pages, never the ones the container/running guests hold.
    free = _read_hugepage_free()
    total = _read_hugepage_nr()
...
def load_image(image, image_tar=None, image_url=None):
    if image_present(image):
        return
    errors = []
    # primary lazy-load path: the image ref carries a registry -> docker pull it.
    # a failed pull falls through to the offline sources below.
    ...
    # offline fallbacks: a tar shipped onto the host, else fetch a tar by url.
