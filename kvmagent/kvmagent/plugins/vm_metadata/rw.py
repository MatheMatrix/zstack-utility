...
Implements vm-metadata-04c-sblk-write-flow.md (write),
vm-metadata-04d-sblk-read-recovery.md (read + recovery + repair).
...
def write_metadata(lv_path, payload, lv_size_getter, lv_extend_func,
    ...
    Args:
        ...
        vm_category:     uint8 VM category (0=REGULAR, 1=APPLIANCEVM, ...)
        vm_uuid:         str – 32-char VM UUID for Header summary
        vm_name:         str – VM display name for Header summary
...
def repair_pending_op(lv_path, lv_size):
    ...
    try:
        ...
        if header.pending_op == PENDING_CONFIG_UPDATE:
            ...
            # Unknown PendingOp 3-255: treat as STORAGE_CHANGE (§4.5)
...
def _read_metadata_fd(fd, lv_size):
    ...
    if header.valid:
        if header.pending_op == PENDING_NONE:
            ...
            # Unknown PendingOp 3-255: treat as STORAGE_CHANGE (conservative)
            logger.warning("Unknown PendingOp value %d, treating as "
                           "STORAGE_CHANGE (conservative path)",
                           header.pending_op)
            return _read_flow_c(fd, header, lv_size)
    else:
...
def _repair_config_update(fd, header, target, lv_size):
    ...
    Tries the header's layout first (old-layout); if target is invalid,
    tries new-layout calculated from current lv_size (§4.5 dual-layout).
    """
    if not target.valid:
        # Try new-layout from current LV size
...
def _try_read_target_new_layout(fd, header, lv_size):
    """Try reading the target slot using a new layout calculated from
    current lv_size (§4.5 dual-layout: old-layout → new-layout fallback).
    """
...
def _recover_from_corrupted_header(fd, header_bytes, lv_size):
    ...
    # Layer 2: Enumerate KNOWN_LV_SIZES for multi-layout probing (§3.3)
    #   Each candidate LV size produces a different layout; we try all of
    #   them (up to 18 I/O probes) to find valid slot data.
    ...
    #   Use current LV size layout as starting point for Slot A position
...
def _brute_force_recovery(fd, lv_size, active_hint=None):
    ...
    Enforces a global timeout of BRUTE_FORCE_TIMEOUT_SEC (§4.1).
    """
