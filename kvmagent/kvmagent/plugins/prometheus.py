            ...
            for info in form.load('id|name|type|value|units|event\n' + sensor_info, sep='|'):
                ...
                if re.match(r"\w*fan(\w*(_|\ )speed|[a-z0-9]\ *\|)\w*", name):
                    ...
                    try:
                        ...
                        logger.warning("failed to convert fan speed value '%s' to float for fan '%s'", value, name)
...
def has_hy_smi():
    r, _, _ = shell.run_without_log("which hy-smi")
    return r == 0

def has_nvidia_smi():
    r, _, _ = shell.run_without_log("which nvidia-smi")
    return r == 0
...
def has_rocm_smi():
    if bash_r("lsmod | grep -q amdgpu") != 0:
        if bash_r("modprobe amdgpu") != 0:
            return False
    r, _, _ = shell.run_without_log("which rocm-smi")
    return r == 0
