...
#       3 in order to save time.
    ...
    def prefix_rebase_backing_files(self, req):
        ...
        # Phase 1: Discovery — 收集所有需要 rebase 的 (current, new_backing) 对
        # 同时收集所有需要激活的 LV 路径（含 image cache）
        rebase_pairs = []       # [(current_path, new_backing_path), ...]
        all_lv_paths = set()    # 所有需要参与操作的 LV 路径
        ...
        # Phase 2: Rebase — 对每个需要修改的 LV 加 EXCLUSIVE 锁并 rebase
        rebased_count = 0
