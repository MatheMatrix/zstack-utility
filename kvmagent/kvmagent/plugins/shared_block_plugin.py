    ...
    def do_takeover(self, cmd):
        ...
        def get_vg_name_by_shared_block_uuid():
            ...
            for _vg_uuid, block_devices in vgsSharedBlockStructs.items():
                if any(not bd.wwid for bd in block_devices):
                    raise Exception("VG %s has block device(s) with empty WWID, abort matching" % _vg_uuid)
        ...
        try:
            ...
            if lock_held:
                # After rename (Step 8), lockspace can be located by the new VG name
                # because LVM associates lockspace via VG metadata, not the original name.
                try:
                    lvm.drop_vg_lock(current_vg)
                except Exception as drop_err:
                    logger.warn("failed to drop vg lock for %s: %s" % (current_vg, str(drop_err)))
                    if current_vg != vg_uuid_on_storage:
                        # Fallback: try original name if rename partially succeeded
                        try:
                            lvm.drop_vg_lock(vg_uuid_on_storage)
                        except Exception as fallback_err:
                            logger.warn("fallback drop vg lock for %s also failed: %s" %
                                        (vg_uuid_on_storage, str(fallback_err)))
            raise
