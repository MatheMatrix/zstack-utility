...
#       3 in order to save time.
    ...
    def prefix_rebase_backing_files(self, req):
        ...
        if not cmd.oldPrefix or not cmd.newPrefix:
            raise Exception("oldPrefix and newPrefix must not be empty")

        # Normalize sharedblock:/ install paths to /dev/ absolute paths
        def _normalize(path):
            if path and path.startswith("sharedblock:/"):
                return translate_absolute_path_from_install_path(path)
            return path

        old_prefix = os.path.normpath(_normalize(cmd.oldPrefix)) + os.sep
        new_prefix = os.path.normpath(_normalize(cmd.newPrefix)) + os.sep

        rebase_pairs = []
        all_lv_paths = set()
        for file_path in [_normalize(p) for p in cmd.filePaths]:
                ...
                if backing.startswith(old_prefix):
                    ...
                    if exists:
                        ...
                        logger.warn("[sblk] new backing %s not accessible, "
                                    "skip entire chain rebase for %s" % (new_backing, file_path))
                        chain_valid = False
                        break
                else:
        ...
        # Collect all LVs and determine lock levels (exclusive > shared).
        # Lock in sorted order to prevent ABBA deadlock across concurrent calls.
        ...
        try:
            for lv_path in sorted_lvs:
                op = lvm.OperateLv(lv_path, shared=lv_lock_levels[lv_path])
                op.__enter__()
                acquired.append(op)
            ...
            for current_path, expected_old_backing, new_backing in rebase_pairs:
                ...
                if actual_backing and actual_backing.startswith("sharedblock:/"):
                    actual_backing = translate_absolute_path_from_install_path(actual_backing)
            ...
            for op in reversed(acquired):
                try:
                    ...
                    logger.warn("[sblk] prefix_rebase: failed to release lock on %s: %s"
                                % (op.abs_path, e))
