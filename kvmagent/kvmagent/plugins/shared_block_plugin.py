...
#       3 in order to save time.
    ...
    def offline_commit_snapshots(self, req):
        ...
        top = translate_absolute_path_from_install_path(cmd.srcPath)
        base = translate_absolute_path_from_install_path(cmd.dstPath)

        with lvm.RecursiveOperateLv(base, shared=True):
            linux.qcow2_commit(top, base)

        _, rsp.size = lvm.get_lv_size(base)
        rsp.totalCapacity, rsp.availableCapacity = lvm.get_vg_size(cmd.vgUuid)
        return jsonobject.dumps(rsp)
