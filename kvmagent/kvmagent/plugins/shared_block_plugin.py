...
#       3 in order to save time.
    ...
    def offline_commit_snapshots(self, req):
        ...
        all_paths = [cmd.base, cmd.top]
        if cmd.topChildrenInstallPathInDb:
            all_paths.extend([translate_absolute_path_from_install_path(children) for children in cmd.topChildrenInstallPathInDb])
        missing_files = [path for path in all_paths if not lvm.lv_exists(path)]
        if missing_files:
            raise Exception("required files missing: " + ", ".join(missing_files))

        # snapshot chain: s1 -> base -> top -> s4 -> volume
        # merge data from top into base, then rebase all topChildren to point to base
        with lvm.RecursiveOperateLv(top, shared=True):
            linux.qcow2_commit(top, base)

            if cmd.topChildrenInstallPathInDb:
                for children in cmd.topChildrenInstallPathInDb:
                    with lvm.RecursiveOperateLv(children, shared=True):
                        if linux.qcow2_get_backing_file(children) != base:
                            linux.qcow2_rebase_no_check(base, children)
