...
class SharedBlockPlugin(kvmagent.KvmAgent):
    ...
    def _run_with_sblk_lock(self, cmd, handler, operation, lock_retry_times=10, lock_retry_interval=(0.1, 1)):
        """Common wrapper for connect/takeover: acquires NamedLock, calls handler, handles errors.

        lock_retry_interval can be a number or a (min, max) tuple for random sleep.
        """
        sblk_lock = lock.NamedLock("sharedblock-%s" % cmd.vgUuid)
        rsp = None
        try:
            self.get_lock(sblk_lock, retry_times=lock_retry_times, retry_interval=lock_retry_interval)
            rsp = handler(cmd)
        except SharedBlockConnectException as e:
            r = AgentRsp()
            r.success = False
            r.error = "can not %s sharedblock primary storage[uuid: %s] on host[uuid: %s], " \
                      "because other thread is %s now" % (operation, cmd.vgUuid, cmd.hostUuid, operation)
    ...
    def connect(self, req):
        ...
        return self._run_with_sblk_lock(cmd, self.do_connect, "connecting")
    ...
    def get_lock(self, sblk_lock, retry_times=10, retry_interval=(0.1, 1)):
        for i in range(retry_times):
            sblk_lock.lock = lock._get_lock(sblk_lock.name)
            if sblk_lock.lock.acquire(False):
                return
            if i < retry_times - 1:
                sleep = random.uniform(*retry_interval) if isinstance(retry_interval, tuple) else retry_interval
                logger.debug("cannot get %s lock, retry %d/%d after %.1fs"
                             % (sblk_lock.name, i + 1, retry_times, sleep))
                time.sleep(sleep)
        raise SharedBlockConnectException("can not get %s lock after %d retries, there is other thread running" % (sblk_lock.name, retry_times))

    def release_lock(self, sblk_lock):
        try:
            sblk_lock.lock.release()
        except Exception as e:
            logger.debug("release lock %s failed (ignored): %s" % (sblk_lock.name, str(e)))
            return
    ...
    def takeover(self, req):
        ...
        global MAX_ACTUAL_SIZE_FACTOR
        MAX_ACTUAL_SIZE_FACTOR = cmd.maxActualSizeFactor
        return self._run_with_sblk_lock(cmd, self.do_takeover, "taking over",
                                           lock_retry_times=10, lock_retry_interval=(0.1, 1))
    ...
    def do_takeover(self, cmd):
        ...
        logger.info("takeover[1/8] prepared %d disk paths: %s" % (len(allDiskPaths), allDiskPaths))

        # Step 2: configure LVM filter
        logger.debug("takeover: calling config_lvm(hostId=%s, vgUuid=%s)" % (cmd.hostId, cmd.vgUuid))
        lvm.config_lvm(cmd.hostId, allDiskPaths, cmd.vgUuid, cmd.hostUuid, DEFAULT_SANLOCK_LV_SIZE,
                       kvmagent.get_host_os_type(), cmd.enableLvmetad)
        logger.info("takeover[2/8] LVM config applied")

        # Step 3: start lock service (lvmlockd + sanlock)
        logger.debug("takeover: calling start_lock_service(ioTimeout=%s)" % cmd.ioTimeout)
        lvm.start_lock_service(cmd.ioTimeout)
        ...
        #   The storage was replicated from the source platform, so the sanlock
        #   lease area (delta lease, GLLK, VGLK, LV leases) still contains the
        #   source host's ownership info.  We must:
        #     1) start_vg_lock  – join the lockspace so we can access the lvmlock LV
        #     2) reset_sanlock_lockspace – reinit all lease areas on disk
        #     3) drop_vg_lock  – discard the now-stale in-memory lockspace state
        #     4) start_vg_lock  – rejoin with a clean lockspace
        #     5) check_gl_lock  – force-enable GL (not auto-enabled after reset)
        logger.debug("takeover: calling check_stuck_vglk_and_gllk()")
        lvm.check_stuck_vglk_and_gllk()
        retry_times_for_checking_vg_lockspace = lvm.get_retry_times_for_checking_vg_lockspace()
        logger.debug("takeover: retry_times_for_checking_vg_lockspace=%s" % retry_times_for_checking_vg_lockspace)

        logger.debug("takeover: calling start_vg_lock(%s, hostId=%s) [before reset]"
                     % (vg_uuid_on_storage, cmd.hostId))
        lvm.start_vg_lock(vg_uuid_on_storage, cmd.hostId, retry_times_for_checking_vg_lockspace)

        logger.debug("takeover: calling reset_sanlock_lockspace(%s)" % vg_uuid_on_storage)
        lvm.reset_sanlock_lockspace(vg_uuid_on_storage)

        logger.debug("takeover: calling drop_vg_lock(%s) [flush stale in-memory state]" % vg_uuid_on_storage)
        lvm.drop_vg_lock(vg_uuid_on_storage)

        logger.debug("takeover: calling start_vg_lock(%s, hostId=%s) [rejoin clean lockspace]"
                     % (vg_uuid_on_storage, cmd.hostId))
        lvm.start_vg_lock(vg_uuid_on_storage, cmd.hostId, retry_times_for_checking_vg_lockspace)
        logger.info("takeover[5/8] sanlock lockspace reset and re-established for %s" % vg_uuid_on_storage)

        # All subsequent steps need the lockspace; wrap in try/except so that
        # on any failure we drop the lockspace, allowing the next host to retry
        # cleanly (idempotent: each step tolerates being re-executed).
        active_vg_uuid = vg_uuid_on_storage
        try:
            # GL lock must be explicitly enabled after lockspace reset;
            # rename_vg and pvchange both require it.
            logger.debug("takeover: calling check_gl_lock() [enable GL before rename]")
            lvm.check_gl_lock()

            # Step 6: rename VG to match the target platform's database UUID.
            # Idempotent: rename_vg returns immediately if new name already exists.
            if vg_uuid_on_storage != cmd.vgUuid:
                logger.debug("takeover: calling rename_vg(%s -> %s)" % (vg_uuid_on_storage, cmd.vgUuid))
                ...
                # vgrename triggers an async lockspace restart under the new name;
                # wait until it is fully ready before any subsequent LVM operation.
                logger.debug("takeover: calling start_vg_lock(%s, hostId=%s) [wait for renamed lockspace]"
                             % (cmd.vgUuid, cmd.hostId))
                lvm.start_vg_lock(cmd.vgUuid, cmd.hostId, retry_times_for_checking_vg_lockspace)
            else:
                logger.info("takeover[6/8] VG name already matches, no rename needed")

            self.clear_stalled_qmp_socket()

            # Step 7: fix PV state.
            # check_missing_pv  – restores PVs marked "missing" (path remapping).
            # reset_pv_uuids   – generates new PV UUIDs to avoid collision with source.
            # Both are idempotent: no-op when already in good state / re-generates UUIDs.
            logger.debug("takeover: calling check_missing_pv(%s)" % cmd.vgUuid)
            lvm.check_missing_pv(cmd.vgUuid)
            logger.debug("takeover: calling reset_pv_uuids(%s)" % cmd.vgUuid)
            ...
            # Idempotent: finds existing INIT_TAG-prefixed tag and replaces it.
            new_tag = "%s::%s::%s::%s" % (INIT_TAG, cmd.hostUuid, time.time(), linux.get_hostname())
            logger.debug("takeover: calling update_vg_tag(%s, new_tag=%s)" % (cmd.vgUuid, new_tag))
        ...
        rsp.hostId = lvm.get_running_host_id(cmd.vgUuid)
        rsp.hostUuid = cmd.hostUuid
        logger.info("takeover completed: vgUuid=%s, hostId=%s" % (cmd.vgUuid, rsp.hostId))
        return jsonobject.dumps(rsp)
