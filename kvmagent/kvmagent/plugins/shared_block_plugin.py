...
class SharedBlockPlugin(kvmagent.KvmAgent):
    ...
    def do_takeover(self, cmd):
        ...
        # Sequence: start_vg_lock (join) -> reset_sanlock_lockspace (reinit leases)
        #   -> drop_vg_lock (flush stale state) -> start_vg_lock (rejoin clean)
        #   -> check_gl_lock (force-enable GL after reset)
        lvm.check_stuck_vglk_and_gllk()
        retry_times = lvm.get_retry_times_for_checking_vg_lockspace()

        # All steps after start_vg_lock are wrapped in try/except: on failure,
        # drop_vg_lock cleans up so the next host can retry.
        ...
        try:
            ...
            if vg_uuid_on_storage != cmd.vgUuid:
                ...
                # Wait for lockspace restart under the new name
            ...
            # Step 7: fix PV state (restore missing PVs, reset PV UUIDs)
            lvm.check_missing_pv(cmd.vgUuid)
