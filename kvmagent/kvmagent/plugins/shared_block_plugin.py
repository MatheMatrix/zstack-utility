...
#       3 in order to save time.
    ...
    def check_vg_state(self, req):
        ...
        def _check(cur_vg):
            ...
            found_vg, invalid_pv_uuid = lvm.get_invalid_pv_uuids(cur_vg, checkIo=False)
            if not found_vg:
                return "vg %s not found" % cur_vg
            elif len(invalid_pv_uuid) != 0:
                return "vg %s is missing pv: %s" % (cur_vg, invalid_pv_uuid)

        sanlock_ls = sanlock.SanlockClientStatusParser(bash.bash_o("sanlock client status -D"))
