...
class StorageDevicePlugin(kvmagent.KvmAgent):
    ...
    def enable_multipath(self, req):
        ...
        config_updater = multipath.MultipathConfigUpdater("/etc/multipath.conf")
        config_updater.set_default_config()
        if config_updater.modified:
            logger.debug("multipath config reset to zstack default config.")
            rsp.configOverwritten = True

        if cmd_dict.get("blacklist", None):
            config_updater.config_section("blacklist", cmd_dict["blacklist"])
