...
''' % (' '.join(GRUB_FILES), ' '.join(GRUB_ROCKY_ENVS))
    ...
    def generate_hygon_mdev_devices(self, req):
        ...
        # This matches the logic: hct_ccp_bind.py -s | grep "drv=hct" | nl -v 0
        # The vendor_idx in mdev corresponds to the index of hct-bound devices
        vendor_idx_to_pci_bdf = {}
        idx = 0
        for device_info in ccp_devices_by_idx:
            # Only map devices that are bound to hct driver
            if device_info['driverStatus'] == 'drv=hct':
