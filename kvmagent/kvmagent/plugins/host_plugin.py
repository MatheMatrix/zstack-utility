...
class HostPlugin(kvmagent.KvmAgent):
    ...
    def _ensure_secret_via_key_agent(self, encrypted_dek, vm_uuid, purpose, key_version, description=None, usage_instance=''):
        if not KEY_AGENT_GRPC_AVAILABLE:
    ...
    def ensure_secret(self, req):
        ...
        secret_uuid, err_code, err_msg = self._ensure_secret_via_key_agent(
            encrypted_dek, vm_uuid, purpose, key_version, description, usage_instance,
        )
        if secret_uuid:
            rsp.success = True
