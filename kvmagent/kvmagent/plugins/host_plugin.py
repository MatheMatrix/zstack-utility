...
class FileSystemUploadTask(UploadTask):
    def __init__(self, task_uuid, installPath):
        super(FileSystemUploadTask, self).__init__(task_uuid, installPath)
    ...
    def create_object(self, slice_offset):
        ...
        self.create_image_if_not_exists(self.installPath)
    ...
    def create_image_if_not_exists(self, install_path):
        ...
        with lock.NamedLock("upload-file-task-%s" % self.task_uuid):
...
''' % (' '.join(GRUB_FILES), reserveSize, pageSize, ' '.join(GRUB_ROCKY_ENVS))
    ...
    def get_upload_progress(self, req):
        ...
        if task.expectedSize == 0:
            ...
            rsp.md5sum = linux.calculate_md5(cmd.dstPath)
            rsp.progress = 100
        else:
