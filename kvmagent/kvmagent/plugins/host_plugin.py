...
''' % (' '.join(GRUB_FILES), reserveSize, pageSize, ' '.join(GRUB_ROCKY_ENVS))
    ...
    def _generate_nvidia_vfio_mdev_devices(self, addr):
    ...
    def _generate_huawei_vfio_mdev_devices(self, addr):
        rsp = GenerateVfioMdevDevicesRsp()
        ...
        type = int(cmd.mdevSpecTypeId, 0)
        spec_path = os.path.join("/sys/bus/pci/devices/", addr, "mdev_supported_types", "vnpu-%d" % type)
    ...
    def generate_vfio_mdev_devices(self, req):
        ...
        if cmd.vendor == VendorEnum.NVIDIA:
            return self._generate_nvidia_vfio_mdev_devices(addr)
        elif cmd.vendor == VendorEnum.HUAWEI:
            return self._generate_huawei_vfio_mdev_devices(addr)
        else:
            rsp.success = False
            rsp.error = "%s is not generate mdev device" % (cmd.vendor)
            return jsonobject.dumps(rsp)
    ...
    def _ungenerate_huawei_vfio_mdev_devices(self, cmd):
        addr = cmd.pciDeviceAddress
        type = cmd.mdevSpecTypeId
        device_path = os.path.join("/sys/bus/pci/devices/", addr, "mdev_supported_types", " vnpu-%d" % type, "devices")
    ...
    def ungenerate_vfio_mdev_devices(self, req):
        ...
        if cmd.vendor == VendorEnum.NVIDIA:
            ...
            rsp.success = False
            rsp.error = "%s is not ungenerate mdev device" % (cmd.vendor)
            return jsonobject.dumps(rsp)
