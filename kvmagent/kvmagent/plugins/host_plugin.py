...
from zstacklib.utils import http, lvm, ceph, form, pci, report, upload_task, upload_task
...
from zstacklib.utils.upload_task import UploadTasks, UploadTask, StorageObject
from zstacklib.utils.upload_task import FileSystemBackend, UploadHandler, StorageBackend
...
class HostPlugin(kvmagent.KvmAgent):
    ...
    KVM_HOST_FILE_UPLOAD_PROGRESS_PATH = "/host/file/upload/progress"
    KVM_HOST_DIR_CAPACITY_PATH = "/host/directory/capacity"
    KVM_HOST_FILE_UNZIP_PATH = "/host/file/unzip"
...
''' % (' '.join(GRUB_FILES), reserveSize, pageSize, ' '.join(GRUB_ROCKY_ENVS))
    ...
    def upload_file(self, req):
        # upload_param = upload_task.get_upload_param(req.headers)
        # task = self.upload_tasks.get_task(req.headers.get(TASK_UUID))
        # upload_task.update_task_by_upload_param(upload_param, task)
        # upload_task.upload_slice(req.body, upload_param, task)
        # if task.allow_completing():
        #     upload_task.complete_upload(task)
    ...
    def unzip_file(self, req):
        ...
        dir_name = os.path.dirname(cmd.installPath)
        md5 = linux.calculate_md5(cmd.installPath)
        newPath = dir_name + '/' + md5
        if not os.path.exists(newPath):
            os.makedirs(newPath)
        if cmd.url.endswith('.tar.gz'):
            shell.run('tar -xzf %s -C %s' % (cmd.installPath, newPath))
        elif cmd.url.endswith('.zip'):
            shell.run('unzip %s -d %s' % (cmd.installPath, newPath))

        return jsonobject.dumps(rsp)

    @kvmagent.replyerror
    def get_dir_capacity(self, req):
        rsp = UploadFileRsp()
        cmd = jsonobject.loads(req[http.REQUEST_BODY])

        current_path = os.path.abspath(cmd.path)
        while current_path != os.path.dirname(current_path):
            if os.path.exists(current_path):
                break
            current_path = os.path.dirname(current_path)
        rsp.totalCapacity, rsp.availableCapacity = linux.get_disk_capacity_by_df(current_path)
    ...
    def start(self):
        ...
        http_server.register_async_uri(self.KVM_HOST_FILE_UNZIP_PATH, self.unzip_file)
        http_server.register_async_uri(self.KVM_HOST_DIR_CAPACITY_PATH, self.get_dir_capacity)
