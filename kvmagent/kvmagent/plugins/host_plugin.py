...
''' % (' '.join(GRUB_FILES), reserveSize, pageSize, ' '.join(GRUB_ROCKY_ENVS))
    ...
    def do_get_sensors(self):
        ...
        def update_sensor_type_by_name():
            ...
            keys_to_remove = [key for key in sensor_type_and_classification_by_name.keys() if key not in sensor_names]
        ...
        for batch in [infos[i:i + 15] for i in range(0, len(infos), 15)]:
            batch_threads = []
            for info in batch:
