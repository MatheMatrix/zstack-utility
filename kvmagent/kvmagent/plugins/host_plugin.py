...
''' % (' '.join(GRUB_FILES), reserveSize, pageSize, ' '.join(GRUB_ROCKY_ENVS))
    ...
    def get_upload_path(self, host):
            UPLOAD_FILE_PATH = "/host/file/upload"
            return 'http://' + host + UPLOAD_FILE_PATH
    ...
    def download_file(self, req):
        ...
        if cmd.url.startswith(UPLOAD_PROTO):
            _prepare_upload()
            rsp.size = 0
            rsp.uploadPath = self.get_upload_path(req[http.REQUEST_HEADER]['Host'])
            return jsonobject.dumps(rsp)
        ...
        rsp.md5sum = linux.calculate_md5(cmd.installPath)
    ...
    def get_upload_progress(self, req):
        ...
        if task.downloadSize == 0:
            ...
            rsp.installPath = self.get_upload_path(req[http.REQUEST_HEADER]['Host'])
