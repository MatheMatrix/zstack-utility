...
from zstacklib.utils import ip as ip_util
...
class TestApplyEipArpRules(_ApplyEipTestBase):
    def test_arpreply_has_source_mac_check(self):
        """arpreply rule in set_gateway_arp_if_needed should include --arp-mac-src."""
        arpreply_cmds = self._cmds_matching("arpreply")
        mac_src_cmds = [
            c for c in arpreply_cmds if "--arp-mac-src" in c and "0:c:29:aa:bb:cc" in c
        ]
        self.assertTrue(
            len(mac_src_cmds) > 0,
            "Expected arpreply rule with --arp-mac-src 0:c:29:aa:bb:cc, got:\n"
            + "\n".join(arpreply_cmds),
        )
    ...
    def test_arp_reply_drop_rules_for_nic_name_chain(self):
        ...
        # Look for: --arp-op Reply --arp-ip-src {NIC_GATEWAY} --arp-mac-src ! {GATEWAY_MAC} -j DROP
        reply_drop_cmds = [
            c
    ...
    def test_arp_reply_filter_in_pri_odev_chain(self):
        """PRI_ODEV filter chain should have ARP Reply rules (ACCEPT for NIC_IP, DROP others)."""
        reply_accept = [
            c
            for c in self.executed_cmds
            if "--arp-op Reply" in c and "-j ACCEPT" in c and "10.0.0.100" in c
        ]
        reply_drop = [
            c
            for c in self.executed_cmds
            if "--arp-op Reply" in c and "-j DROP" in c and "eip-123456789_o-gw" in c
        ]
        self.assertTrue(
            len(reply_accept) > 0,
            "Expected ARP Reply ACCEPT for NIC_IP in PRI_ODEV chain",
        )
        self.assertTrue(
            len(reply_drop) > 0, "Expected ARP Reply DROP in PRI_ODEV chain"
        )
