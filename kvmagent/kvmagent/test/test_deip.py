...
class TestZSTAC86874EipPublicInterfaceState(unittest.TestCase):
    ...
    def test_public_and_private_announcements_start_before_waiting(self):
        self.mock_iproute.IpNetnsShell.list_netns.return_value = [
            "br_eth0_192_168_1_100"
        ]
        self.mock_iproute.IpNetnsShell.return_value.get_mac.return_value = (
        ...
        def run_in_thread(target, args=(), kwargs=None):
            command = args[0]
            operations.append(("start", command))
            task = mock.MagicMock()
            task.join.side_effect = lambda: operations.append(("join", command))
            return task
        ...
        self.assertEqual(["start", "start", "join", "join"],
                         [operation for operation, _ in operations])
        self.assertIn("arping -q -A", operations[0][1])
        self.assertIn("arping -q -U", operations[1][1])
