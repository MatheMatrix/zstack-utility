...
class TestSharedBlockPlugin(TestCase, SharedBlockPluginTestStub):
    ...
    def test_sharedblock_rollback_memory_snapshot(self):
        ...
        vm = vm_plugin.Vm()
        mount_path = vm.make_block_device_for_memory_snapshot("/dev/%s/%s" % (vgUuid, all_volume[0]))
        vm.rollback_memory_snapshot("/dev/%s/%s" % (vgUuid, all_volume[0]))

        # check mount path is not exist
        r, _ = bash.bash_ro("mount | grep %s" % mount_path)
        self.assertEqual(r, 1, "mount path %s is exist" % mount_path)

        # check volume is not exist
        r, _ = bash.bash_ro("ls /dev/%s | grep %s" % (vgUuid, all_volume[1]))
        self.assertEqual(r, 1, "volume %s is exist" % all_volume[1])
