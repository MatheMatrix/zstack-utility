...
class TestSblkMetadataApi(TestCase, SharedBlockPluginTestStub):
    ...
    def _test_read_flow_b_phase2_complete(self, handler):
        ...
        has matching seq_num) => NEED_REPAIR with target payload."""
        vm_uuid = uuid_mod.uuid4().hex
        meta_path = self._meta_path(vm_uuid)
    ...
    def _test_read_flow_b_phase2_incomplete(self, handler):
        ...
        still has old data) => NEED_REPAIR with active slot payload,
        repair=clear_pending_op."""
        vm_uuid = uuid_mod.uuid4().hex
        meta_path = self._meta_path(vm_uuid)
        ...
        try:
            ...
            self.assertEqual(result.status, ReadStatus.NEED_REPAIR)
            self.assertEqual(result.repair_action, 'clear_pending_op')
    ...
    def _test_read_flow_c_phase2_incomplete(self, handler):
        ...
        STORAGE_CHANGE_INCOMPLETE, repair=full_refresh_required."""
        vm_uuid = uuid_mod.uuid4().hex
        meta_path = self._meta_path(vm_uuid)
        ...
        try:
            ...
            self.assertEqual(result.repair_action, 'full_refresh_required')
    ...
    def _test_read_corrupted_header_returns_corrupted(self, handler):
        """Corrupted header => ReadStatus.CORRUPTED with full_refresh action."""
        vm_uuid = uuid_mod.uuid4().hex
        meta_path = self._meta_path(vm_uuid)
        ...
        try:
            ...
            self.assertEqual(result.repair_action, 'full_refresh')
        finally:
            self._cleanup_lv(meta_path)
    ...
    def _test_read_flow_a_active_corrupted_fallback_inactive(self, handler):
        ...
        DEGRADED with inactive payload."""
        vm_uuid = uuid_mod.uuid4().hex
        meta_path = self._meta_path(vm_uuid)
