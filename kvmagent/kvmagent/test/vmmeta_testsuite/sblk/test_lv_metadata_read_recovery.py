...
class TestLvMetadataReadRecovery(TestCase, SharedBlockPluginTestStub):
    ...
    def _test_corrupted_header_recovery(self):
        ...
        try:
            ...
            self.assertEqual(result.status, ReadStatus.RECOVERED,
                             "expected RECOVERED after header zeroed (brute force recovery), "
                             "got %s: %s" % (result.status, result.error))
