...
from zstacklib.utils.lv_protocol import (
    HEADER_BLOCK_SIZE,
    PENDING_CONFIG_UPDATE, PENDING_STORAGE_CHANGE,
    INITIAL_LV_SIZE,
    ReadStatus,
    build_header,
    build_slot,
...
from zstacklib.utils.lv_metadata import (
    ...
    calculate_slot_layout, open_lv, aligned_pwrite,
)
...
class TestLvMetadataReadRecovery(TestCase, SharedBlockPluginTestStub):
    ...
    def _test_corrupted_header_returns_corrupted(self):
        lv_path = self._create_lv('tr_hdr')
        try:
            with lvm.OperateLv(lv_path, shared=False):
                self._write(lv_path, '{"data":true}')
    ...
    def _test_all_data_zeroed(self):
        ...
        try:
            with lvm.OperateLv(lv_path, shared=False):
                ...
                # Explicitly zero header, slot A and slot B to avoid
                # residual data on reused PEs.
                bash.bash_errorout(
                    'dd if=/dev/zero of=%s bs=%d count=1 conv=notrunc oflag=direct'
                    % (lv_path, HEADER_BLOCK_SIZE))
                bash.bash_errorout(
                    'dd if=/dev/zero of=%s bs=4096 count=%d seek=%d conv=notrunc oflag=direct'
                    % (lv_path, layout.slot_a_capacity // 4096, layout.slot_a_offset // 4096))
                bash.bash_errorout(
                    'dd if=/dev/zero of=%s bs=4096 count=%d seek=%d conv=notrunc oflag=direct'
                    % (lv_path, layout.slot_b_capacity // 4096, layout.slot_b_offset // 4096))
    ...
    def test_lv_metadata_read_recovery(self):
        ...
        self._test_corrupted_header_returns_corrupted()
