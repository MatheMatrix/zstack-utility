"""lv_metadata: sblk_prefix_rebase_backing_files -- chain walk, OperateLv lock levels,
sorted lock order, sharedblock:/ normalize, backing-changed-since-discovery skip, exception releases locks.

Real LV I/O on iSCSI-backed shared VG. Each test scenario is a _test_xxx sub-method
called from a single entry point."""
...
class TestVmmetaPrefixRebase(TestCase, SharedBlockPluginTestStub):
    ...
    def _test_multi_level_chain(self):
        ...
        vol_lv = _create_lv('pr_cvol_%s' % tid)
        try:
            _create_qcow2_on_lv(base_lv)
            ...
            self.assertIsNone(_get_backing(base_lv))
    ...
    def _test_concurrent_no_deadlock(self):
        ...
        vol_lvs = [_create_lv('pr_ccvol_%s_%d' % (tid, i)) for i in range(num_vols)]
        try:
            ...
            errors = []
            barrier = threading.Barrier(num_vols)
