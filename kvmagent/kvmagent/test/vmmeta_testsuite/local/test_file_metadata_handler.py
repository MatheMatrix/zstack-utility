"""file_metadata_handler: FileBasedMetadataHandler -- _do_write, _do_get, _do_scan, _do_cleanup.

Real file I/O on tempfile.mkdtemp(). No mocks.
...
class TestFileMetadataHandler(TestCase):
    """FileBasedMetadataHandler _do_write / _do_get / _do_scan / _do_cleanup."""
    ...
    def _test_scan_missing_directory(self):
        entries = self.handler._do_scan('/tmp/nonexistent_vmmeta_dir_xyz')
        self.assertEqual(len(entries), 0)
