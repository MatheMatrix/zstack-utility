...
def create_ovmf_symlinks():
    ...
    The rpm is built into the alinux4 ISO repo via mkrepo_alinux4 + comps_alinux4.xml.tmpl,
    so it must be installable from the local zstack-local/zstack-mn repos.
    No external network fallback - offline deployment is the product baseline.

    Two independent idempotent steps:
      1) yum install -y is itself idempotent; running it unconditionally also self-heals
         a previous half-installed state.
      2) Symlink check/recreate runs every time so a missing/broken secboot link is
         repaired even when the rpm is already present.
    """
    ...
    command = (
        ...
        # 2) (re)create secboot symlinks unconditionally; ln -sf is idempotent
