...
def repair_rpmdb_if_damaged(host_post_info):
    cmd = "out=$(timeout --kill-after=10 60 yum --disablerepo=* --enablerepo=zstack-local list 2>&1 >/dev/null); rc=$?; " \
          ...
          "ps -eo pid,comm,args --no-headers | awk '(\\$2==\\\"yum\\\" || \\$2 ~ /^python/) && /enablerepo=zstack-local/ {print \\$1}' | xargs -r kill -9; " \
          "sleep 1; " \
