"""IP6Tables rule management for IPv6.

Provides classes for parsing, modifying, and restoring ip6tables rules.
"""
...
from zstacklib.network.firewall.iptables import IPTableTable, IPTableChain, IPTableRule
from zstacklib.network.firewall.exceptions import IPTablesError

logger = log.get_logger(__name__)

_ip6tablesUseLock: Optional[bool] = None


def get_ip6tables_cmd(command: Optional[str] = None) -> str:
    """Get the appropriate ip6tables command with lock flag if supported."""
    global _ip6tablesUseLock

    if _ip6tablesUseLock is None:
        _ip6tablesUseLock = shell.run("ip6tables -w -nL > /dev/null") == 0

    if command is None:
        return "ip6tables -w" if _ip6tablesUseLock else "ip6tables"
    elif command == "restore":
        return "ip6tables-restore -w" if _ip6tablesUseLock else "ip6tables-restore"
    return "ip6tables"


class IP6Tables(Node):
    """Main class for managing ip6tables rules (IPv6).
    
    Supports parsing from ip6tables-save output and restoring via ip6tables-restore.
    """
...
''' % (str(e), str(res), content)
    ...
    @staticmethod
    def from_iptables_save() -> 'IP6Tables':
        """From iptables save."""
        ipt = IP6Tables()
        ipt.iptables_save()
        return ipt
...
def from_ip6tables_save() -> IP6Tables:
    """Create IP6Tables instance from current ip6tables-save output."""
