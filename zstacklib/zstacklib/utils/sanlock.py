...
def lease_corrupted_err(rv):
    return rv in LEASE_CORRUPTED_ERR

def lvmlock_lv_exists(vg_name):
    return os.path.exists("/dev/mapper/{}-lvmlock".format(vg_name))
...
'''
def vglk_lvb_invalid(lvb, vg_name):
...
@ignoreerror
def check_vglk_metadata(vg_name):
    if not lvmlock_lv_exists(vg_name):
        return

    with lock.NamedLock("check-vglk-{}-lease".format(vg_name)):
        ...
        for line in o.strip().splitlines():
            ...
            if line.startswith("read done "):
                ...
                if lease_corrupted_err(read_rv):
                    logger.debug("vglk lease corrupted, cmd {}, err\n{}, reinit it by vgck.".format(cmd, o))
                    bash.bash_r("vgck {} --lockopt repairvg".format(vg_name))
                    break

            if line.startswith("lvb "):
                lvb = int(line.split()[-1], 16)
                if vglk_lvb_invalid(lvb, vg_name):
                    logger.debug("vglk lvb invalid, cmd {}, err\n{}, reinit it by sanlock directly.".format(cmd, o))
...
def check_lvmlock_io(vg_name):
    return bash.bash_r("dd if=/dev/mapper/{}-lvmlock of=/dev/null bs=1M count=1 iflag=direct".format(vg_name))
...
def get_sector_size(vg_uuid):
    ...
    if sector_size and sector_size in (SECTOR_SIZE_512, SECTOR_SIZE_4K):
        sector_size_cache.update({vg_uuid:sector_size})
        logger.debug("read sector size[{0}] from lvm lockspace info for vg {1}".format(sector_size, vg_uuid))
        return sector_size
