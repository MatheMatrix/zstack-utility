...
def is_gpu_associated_device(addr):
    for pci_device_address in collect_pci_devices_with_dependencies(addr):
        pciDevice = get_cached_device(pci_device_address)
        if pciDevice and is_gpu(pciDevice.type):
            return True
    return False
