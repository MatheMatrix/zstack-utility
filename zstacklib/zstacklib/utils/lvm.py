...
def reset_sanlock_lockspace(vg_uuid):
    """Reset copied sanlock lockspace: clear all host slots and re-init resource leases."""
    lockspace_path = "/dev/mapper/%s-lvmlock" % vg_uuid

    # Safety check: refuse to reinit if the lockspace is currently active on this host,
    # to avoid invalidating sanlock leases held by other hosts (risk of data corruption).
    ...
    # Activate lvmlock LV with --sysinit to bypass lvmlockd (lockspace not started yet)
    r, o, e = bash.bash_roe("timeout -s SIGKILL 30 lvchange -ay --sysinit %s/lvmlock" % vg_uuid)
    if r != 0:
        raise Exception("failed to activate lvmlock LV for %s: %s" % (vg_uuid, e))

    # Re-init lockspace (clears all host delta lease slots)
    r, o, e = bash.bash_roe(
    ...
    # Re-init VGLK and GLLK resource leases.
    # If delta lease was cleared but resource lease init fails, lockspace is half-initialized.
    # Log explicit recovery steps so the operator knows what to do.
    try:
        ...
        bash.bash_roe("timeout -s SIGKILL 30 lvchange -an %s/lvmlock" % vg_uuid)
        raise
