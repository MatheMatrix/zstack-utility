...
@bash.in_bash
def restart_lvmlockd():
    """Restart lvmlockd so it rescans all VGs.

    Needed after vgrename --nolocking which bypasses lvmlockd, leaving it
    with a stale VG name cache. Without restart, vgchange --lock-type sanlock
    silently fails to create the lvmlock LV for the renamed VG.
    Adopt file preserves lock state for other VGs across the restart.
    """
    write_lvmlockd_adopt_file()
    stop_lvmlockd()
    linux.rm_file_force(LVMLOCKD_SOCKET)
    cmd = shell.ShellCmd("timeout 30 systemctl start %s" % get_lvmlockd_service_name())
    cmd(is_exception=True)
    logger.debug("lvmlockd restarted (with adopt file)")
...
def reset_sanlock_lockspace(vg_uuid):
    ...
    def repair(uuid, offset):
        bash.bash_errorout("sanlock direct init -r lvm_%s:%s:%s:%s" % (vg_uuid, uuid, lockspace_path, offset))
