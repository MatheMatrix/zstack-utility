...
def rename_vg(old_vg_uuid, new_vg_uuid):
    ...
    @linux.retry(times=6, sleep_time=10)
    def do_vg_rename():
        r, o, e = bash.bash_roe("vgrename %s %s" % (old_vg_uuid, new_vg_uuid))
        if r != 0:
...
def reset_sanlock_lockspace(vg_uuid):
    ...
    r, o, e = bash.bash_roe("lvchange -ay --sysinit %s/lvmlock" % vg_uuid)
    if r != 0:
        raise Exception("failed to activate lvmlock LV for %s: %s" % (vg_uuid, e))

    # Re-init lockspace (clears all host delta lease slots)
    r, o, e = bash.bash_roe(
        "sanlock direct init -s lvm_%s:0:%s:0" % (vg_uuid, lockspace_path))
    if r != 0:
...
def reset_pv_uuids(vg_uuid):
    ...
    for pv_line in pvs_output:
        ...
        if not pv_name:
            continue
        r, o, e = bash.bash_roe("pvchange --uuid %s" % pv_name)
        if r != 0:
            raise Exception("failed to reset PV UUID for %s: %s\n"
                            "succeeded PVs: %s" % (pv_name, e, succeeded))
        succeeded.append(pv_name)
        logger.debug("reset PV UUID for %s successfully" % pv_name)


@bash.in_bash
def update_vg_tag(vg_uuid, host_uuid):
    """Update VG INIT_TAG: replace source platform creator info with current platform."""
    r, o, e = bash.bash_roe("vgs --nolocking -t -o tags --noheadings %s" % vg_uuid)
    if r != 0:
    ...
    new_tag = "%s::%s::%s::%s" % (INIT_TAG, host_uuid, time.time(), linux.get_hostname())
