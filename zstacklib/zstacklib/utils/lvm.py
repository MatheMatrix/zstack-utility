...
@bash.in_bash
def remove_device_map_for_vg(vgUuid): # type: (str) -> None
...
def config_lvm(host_id, all_disk_paths, vg_uuid, host_uuid, default_sanlock_lv_size, host_os_type,
    ...
    old_local_config = linux.read_file(LVM_LOCAL_CONFIG_FILE)
    old_global_config = linux.read_file(LVM_CONFIG_FILE)
    local_diff = list(difflib.unified_diff(
        old_local_config.splitlines() if old_local_config is not None else [],
        new_config.splitlines()))
    global_diff = list(difflib.unified_diff(
        old_global_config.splitlines() if old_global_config is not None else [],
        new_config.splitlines()))
    if len(local_diff) == 0 and len(global_diff) == 0:
...
def rename_vg(old_vg_uuid, new_vg_uuid):
    ...
    def do_vg_rename():
        r, o, e = bash.bash_roe(
            "timeout -s SIGKILL %s vgrename --nolocking %s %s" %
        ...
        if vg_exists(old_vg_uuid) and not vg_exists(new_vg_uuid):
            raise Exception("vg rename command executed but old VG %s still exists and new VG %s not found, retrying...")
...
def reset_pv_uuids(vg_uuid):
    ...
    for pv_line in pvs_output:
        ...
        if not pv_name:
            continue
        r, o, e = bash.bash_roe("timeout -s SIGKILL 30 pvchange --yes --uuid %s" % pv_name)
        if r != 0:
...
def reset_sanlock_lockspace(vg_uuid):
    """Reset sanlock lockspace: clear all host delta lease slots and reinit VGLK/GLLK.

    This is used in takeover to eliminate stale host entries from the old (dead) host.
    After reinit, sanlock no longer tracks the old host. VGLK/GLLK are force-reinited
    so start_vg_lock + check_gl_lock can proceed cleanly.

    NOTE: LV resource leases are NOT reinited here because the sanlock resource name
    for LV locks is an internal lvmlockd identifier (not the LV name). Reiniting with
    the wrong name corrupts the lease (error -221). Instead, vgrename should use
    --nolocking to bypass lvmlockd and skip LV lock acquisition entirely.

    Prerequisites:
      - The /dev/mapper/<vg>-lvmlock device must exist (e.g. from a prior start_vg_lock).
      - The lockspace must NOT be active (call drop_vg_lock first).
    """
    lockspace_path = "/dev/mapper/%s-lvmlock" % vg_uuid

    # Safety: refuse to reinit an active lockspace.
    r, o, _ = bash.bash_roe("sanlock client status | grep 'lvm_%s'" % vg_uuid)
    if r == 0 and o.strip():
        raise Exception("lockspace lvm_%s is currently active, refusing to reinit" % vg_uuid)

    # Ensure lvmlock device exists. If already active (from prior start_vg_lock), skip.
    if not os.path.exists(lockspace_path):
        r, o, e = bash.bash_roe("timeout -s SIGKILL 30 lvchange -ay --nolocking %s/lvmlock" % vg_uuid)
        if r != 0:
            raise Exception("failed to activate lvmlock LV for %s: %s" % (vg_uuid, e))
    ...
    # Force reinit VGLK and GLLK resource leases to clear stale holders.
    try:
        sanlock.force_init_vglk(vg_uuid)
        sanlock.force_init_gllk(vg_uuid)
    except Exception as e:
        logger.error("CRITICAL: sanlock lockspace for VG %s is half-initialized. "
                     "Delta lease was cleared but resource lease init failed: %s. "
                     "Manual recovery: run 'sanlock direct init -r lvm_%s:...' for VGLK/GLLK, "
                     "or re-run the takeover operation." % (vg_uuid, str(e), vg_uuid))
        bash.bash_roe("timeout -s SIGKILL 30 lvchange -an --nolocking %s/lvmlock" % vg_uuid)
        raise

    logger.debug("sanlock lockspace for VG %s has been reset" % vg_uuid)
