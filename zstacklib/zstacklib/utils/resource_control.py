...
class ResourceControlManager(object):
    ...
    def restart(self, handles):
        ...
        self._systemctl(['stop'] + units, 120)
        self._systemctl(['start'] + units, 120)
        for unit in units:
            if self._systemd_properties(unit).get('ActiveState') != 'active':
                raise ResourceControlError('SYSTEMD_UNIT_RESTART_FAILED')
    ...
    def validate_cpu_set(self, cpu_set, enabled=True):
        try:
            normalized = self._normalize(cpu_set)
    ...
    def _pid_from_handle(self, handle):
        ...
        if info.st_mode & (stat.S_IWGRP | stat.S_IWOTH) and not (
                info.st_uid == 0 and process_uid == 0 and expected):
