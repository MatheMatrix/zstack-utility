...
def sblk_prefix_rebase_backing_files(file_paths, old_prefix, new_prefix,
                                     normalize_path, lvm_module):
    ...
    for file_path in [normalize_path(p) for p in file_paths]:
        ...
        while current and current not in visited:
            ...
            if backing.startswith(old_prefix):
                ...
                try:
                    ...
                    logger.debug("[sblk] failed to check new backing %s: %s",
                                 new_backing, e)
                    exists = False
                ...
                if exists:
                    ...
                    logger.warn("[sblk] new backing %s not accessible, "
                                "skip entire chain rebase for %s"
    ...
    try:
        for lv_path in sorted_lvs:
            op = lvm_module.OperateLv(lv_path, shared=lv_lock_levels[lv_path])
            op.__enter__()
            acquired.append(op)
        ...
        for current_path, expected_old_backing, new_backing in rebase_pairs:
            ...
            if actual_backing != expected_old_backing:
                logger.warn("[sblk] prefix_rebase: backing of %s changed "
                            ...
                            % (current_path, expected_old_backing,
                               actual_backing))
                continue

            qcow2_rebase_no_check(new_backing, current_path)
            rebased_count += 1
            logger.info("[sblk] prefix_rebase: rebased %s -> %s"
                        % (current_path, new_backing))
    finally:
        for op in reversed(acquired):
            try:
                op.__exit__(None, None, None)
            except Exception as e:
                logger.warn("[sblk] prefix_rebase: failed to release lock "
                            "on %s: %s" % (op.abs_path, e))
