"""LV Metadata business logic: layout, lifecycle, read/write, and handler.

Consolidates lv_manager + lv_rw + lv_metadata_handler.
Depends on lv_protocol (single direction, no cycles).
Python 2 compatible.
"""
...
def get_metadata_status(lv_path, lv_size):
    ...
    try:
        ...
        return {
            ...
            'summary_valid': header.summary_valid,
        }
    finally:
        os.close(fd)
...
class SblkMetadataHandler(VmMetadataHandler):
    ...
    def _ensure_metadata_lv(self, metadata_path):
        ...
        initialize_metadata_lv(metadata_path, INITIAL_LV_SIZE)
    ...
    def _do_get_all(self, metadataPath):
        # metadataPath is /dev/{vgUuid}/{vmUuid}_vmmeta; extract vgUuid from parent dir
        ...
        for item in metadata_lvs:
            ...
            try:
                with lvm.OperateLv(lv_path, shared=True):
                    read_result = read_metadata(lv_path, lv_size)
                if read_result.is_usable():
                    payload = read_result.payload.decode('utf-8') \
                        if isinstance(read_result.payload, bytes) else read_result.payload
                    if payload:
    ...
    def _do_cleanup(self, metadataPath):
        ...
        try:
            ...
            raise Exception("failed to cleanup metadata LV %s: %s" % (metadataPath, e))
