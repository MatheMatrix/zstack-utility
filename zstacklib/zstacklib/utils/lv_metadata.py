...
def _write_fresh(lv_path, payload_bytes, lv_size,
    ...
    fd = open_lv(lv_path)
    try:
        # Clear inactive Slot B head block to prevent stale data
        # from being misinterpreted during degraded/recovery reads.
        zero_block = b'\x00' * ALIGNMENT
        aligned_pwrite(fd, zero_block, layout.slot_b_offset)
