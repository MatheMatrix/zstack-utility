...
class SblkMetadataHandler(VmMetadataHandler):
    ...
    def _do_write(self, metadataPath, metadata, vmUuid, vmName, vmCategory, architecture, schemaVersion):
        ...
        # Binary protocol requires int for struct.pack('>Q', schema_version).
        # Java sends schemaVersion as String; coerce to int at the boundary.
        try:
            schema_version_int = int(schemaVersion) if schemaVersion else 0
        except (ValueError, TypeError):
            schema_version_int = 0

        try:
            vm_category_int = int(vmCategory) if vmCategory else 0
        except (ValueError, TypeError):
            vm_category_int = 0
