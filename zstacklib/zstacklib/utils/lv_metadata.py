...
def initialize_metadata_lv(lv_path, lv_size):
    ...
    try:
        ...
        # interfering with _brute_force_recovery (which scans every 4KB block).
        # slot_b_capacity is always a multiple of ALIGNMENT (4KB).
        zero_block = b'\x00' * ALIGNMENT
        off = layout.slot_b_offset
        end = off + layout.slot_b_capacity
        while off < end:
            aligned_pwrite(fd, zero_block, off)
            off += ALIGNMENT
...
def scan_metadata_lvs(vg_uuid, lv_list_func):
    ...
    for lv_name, lv_path, lv_size in lv_list_func(vg_uuid):
        if lv_name.endswith(LV_METADATA_SUFFIX):
            ...
            if not re.match(r'^[0-9a-fA-F]{32}$', vm_uuid):
                continue
            result.append({
                'vm_uuid': vm_uuid,
                'lv_path': lv_path,
...
def write_metadata(lv_path, payload, lv_size_getter, lv_extend_func,
    ...
    fd = open_lv(lv_path)
    try:
        # Phase 1: Mark Intent (keep OLD summary fields so abort is consistent)
        phase1 = build_header(
            active_slot=header.active_slot,
            pending_op=op_type,
...
def _extract_summary_from_payload(payload):
    """Best-effort extraction of summary fields from a slot's JSON payload.

    During Phase 3 completion in repair, the Phase 1 header may carry stale
    summary fields.  If the payload is valid JSON with summary keys, return
    them so the repaired header is consistent with the actual payload.
    Returns an empty dict on failure (caller falls back to header values).
    """
    try:
...
def _repair_config_update(fd, header, target, lv_size):
    ...
    if target.valid and target.seq_num == header.write_sequence:
        # Complete Phase 3.
        # Phase 1 header may carry stale summary fields (vmName, vmCategory,
        # etc.) because the write protocol only updates them in Phase 3.
        # Try to extract fresh summary from the target slot's payload JSON.
        summary = _extract_summary_from_payload(target.payload)
        h = build_header(
            ...
            slot_b_capacity=layout.slot_b_capacity,
            last_update_time=current_epoch_ms(),
            schema_version=summary.get('schemaVersion', header.schema_version),
            vm_category=summary.get('vmCategory', header.vm_category),
            vm_uuid=summary.get('vmUuid', header.vm_uuid),
            vm_name=summary.get('vmName', header.vm_name),
            architecture=summary.get('architecture', header.architecture))
        aligned_pwrite(fd, h, 0)
        return True, "Completed Phase 3 for config update"
    else:
        # Target invalid -> abort incomplete write, clear PendingOp.
        # But first verify active slot is still readable; if both slots
        # are corrupt, we must NOT clear pending_op as that would mask
        # the corruption and prevent a proper full_refresh.
        active = _read_active_slot(fd, header)
        if not active.valid:
            return False, ("CONFIG_UPDATE pending, target data lost, "
                           "active slot also corrupted. Must execute full_refresh.")
    ...
    while offset < lv_size:
        ...
        while pos + SLOT_HEADER_STRUCT_SIZE <= len(data):
            if data[pos:pos + 4] == SLOT_MAGIC_BYTES:
                ...
                slot_data = data[pos:]
                # If the data remaining in this chunk may be too small for a
                # full slot, do a targeted re-read from the LV to cover the
                # slot header + payload + checksum.  We peek at payload_len
                # from the header to decide.
                if remaining >= SLOT_HEADER_STRUCT_SIZE:
                    _hdr = struct.unpack(SLOT_HEADER_FORMAT,
                                         data[pos:pos + SLOT_HEADER_STRUCT_SIZE])
                    needed = SLOT_HEADER_STRUCT_SIZE + _hdr[4] + 32  # hdr + payloadLen + sha256
                    if remaining < needed and actual_offset + needed <= lv_size:
                        try:
                            slot_data = aligned_pread(fd, needed, actual_offset)
                        except Exception:
                            pass  # fall through with truncated data, parse_slot will reject
                slot = parse_slot(slot_data,
...
def _write_fresh(lv_path, payload_bytes, lv_size,
    ...
    try:
        ...
        # from being found by _brute_force_recovery (scans every 4KB block).
        zero_block = b'\x00' * ALIGNMENT
        off = layout.slot_b_offset
        end = off + layout.slot_b_capacity
        while off < end:
...
class SblkMetadataHandler(VmMetadataHandler):
    ...
    def _initialize_if_needed(self, metadata_path, lv_size):
        ...
        write_metadata() handle it -- it can recover data from slots or fall
        back to a fresh write, whereas initialize_metadata_lv() would
        unconditionally wipe potentially recoverable slot data.
        """
        ...
        # A non-zero but invalid header means corruption -- let write_metadata
        # attempt slot recovery instead of destroying the data.
        if header_bytes == b'\x00' * HEADER_BLOCK_SIZE:
            ...
            logger.warn("metadata LV %s has invalid header but non-zero data, "
                        "skipping initialization to preserve recoverable slots",
                        metadata_path)
