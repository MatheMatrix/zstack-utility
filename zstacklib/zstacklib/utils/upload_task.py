...
class UploadHandler(object):
    ...
    def get_upload_param(headers):
        ...
        elif 'X-APIID-UUID' in headers:
            up.task_uuid = headers['X-APIID-UUID']
    ...
    def get_upload_task(self, param):
        ...
        if task is None:
            raise Exception('image not found %s' % param.task_uuid)

        if task.lastError:
            self._fail_task(task, task.lastError)

        if task.completed:
            raise Exception('image[uuid: %s] upload has completed' % param.task_uuid)
