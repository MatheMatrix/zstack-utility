...
class UploadTask(object):
    def __init__(self, imageUuid, dstPath):
...
class UploadTasks(object):
    ...
    def add_task(self, task):
        with lock.lock(self.lock_name):
            if len(self.tasks) >= self.MAX_RECORDS:
                self._expunge_oldest_task()
            self.tasks[task.resource_uuid] = task

    def get_task(self, resource_uuid):
        with lock.lock(self.lock_name):
            return self.tasks.get(resource_uuid)

    def _expunge_oldest_task(self):
        oldest_uuid, oldest_time = '', linux.get_current_timestamp()
        for uuid, task in self.tasks.iteritems():
            if task.last_op_time < oldest_time:
                oldest_uuid, oldest_time = uuid, task.last_op_time

        if oldest_uuid != '':
            del self.tasks[oldest_uuid]
            logger.info("Expunged oldest task: %s", oldest_uuid)
...
class UploadHandler(object):
    ...
    def get_upload_param(headers):
        ...
        up = UploadParam()
        up.resource_uuid = headers['X-IMAGE-UUID']

        # 通用上传参数
        up.total_size = get_long_field('X-TOTAL-SIZE')
    ...
    def update_task_by_upload_param(self, param, task):
        if task.lastError:
        ...
        if param.slice_index == 0:
            ...
            err = task.backend.check_capacity(task.expectedSize)
            if err:
    ...
    def upload_slice(self, entity, param):
        """处理分片上传"""
        ...
        try:
            self.stream_body(entity, boundary, param)
        except Exception, e:
    ...
    def handle_upload(self):
        ...
        task = self.upload_tasks.get_task(upload_param.image_uuid)
        if not task:
            raise Exception("Task not found for resource UUID: %s" % upload_param.image_uuid)

        self.update_task_by_upload_param(upload_param, task)

        self.upload_slice(self.req.body, upload_param)
