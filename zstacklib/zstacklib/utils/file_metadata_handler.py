...
class FileBasedMetadataHandler(VmMetadataHandler):
    ...
    def _evict_unlocked_entries(self):
        ...
        to_remove = []
        for path, lock in self._lock_map.items():
...
def qcow2_prefix_rebase_backing_files(file_paths, old_prefix, new_prefix):
        ...
        while current and current not in visited:
            ...
            if backing_norm.startswith(old_prefix):
                ...
                if not os.path.exists(new_backing):
                    logger.warn("new backing %s not exist, skip entire chain "
                                "rebase for %s" % (new_backing, file_path))
                    chain_valid = False
                    break
    ...
    sorted_lock_targets = sorted(imagecache_paths)
    acquired_locks = []

    try:
        for img_path in sorted_lock_targets:
            lock_path = _get_rebase_lock_path(img_path)
            logger.debug("acquiring rebase lock: %s" % lock_path)
        ...
        for current_path, new_backing in all_rebase_pairs:
            ...
            if actual_backing:
                actual_norm = os.path.normpath(actual_backing) if os.path.isabs(actual_backing) else actual_backing
                ...
                if actual_norm != expected_old_norm:
                    logger.info("backing of %s changed since discovery "
                                "(expected %s, got %s), skip rebase"
                                % (current_path, expected_old_norm, actual_norm))
                    continue
        ...
        for fl in reversed(acquired_locks):
            try:
                ...
                logger.warn("failed to release rebase lock: %s" % e)
