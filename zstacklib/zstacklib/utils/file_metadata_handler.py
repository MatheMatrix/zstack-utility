...
class FileBasedMetadataHandler(VmMetadataHandler):
    ...
    def _do_cleanup_all(self, metadataDir):
        """Delete all VM metadata files under metadataDir.

        Only files whose names match the strict metadata pattern
        ``<32-hex-uuid>.vmmeta`` (and the ``.tmp`` / ``.summary`` /
        ``.summary.tmp`` companions) are removed.  All other files in
        the directory are left untouched, so a misconfigured metadataDir
        cannot trigger catastrophic data loss.
        """
        ...
        for fname in names:
            ...
            try:
                os.remove(fpath)
                removed_any = True
                # Count only the canonical .vmmeta entries; companion files
                # (.tmp/.summary/.summary.tmp) belong to the same VM and
