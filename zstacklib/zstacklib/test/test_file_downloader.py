...
def _file_download_cmd(api_id, source, destination):
    cmd = _download_cmd(api_id)
    cmd.url = 'file://%s' % source
    cmd.urlScheme = 'file'
    cmd.installPath = destination
    return cmd
...
class TestFileDownloader(unittest.TestCase):
    ...
    def test_http_download_error_redacts_url_password(self):
        cmd = _download_cmd('redacted-file-download-api')
        cmd.url = 'http://user:secret@example.com/image.qcow2'
        downloader = file_downloader.FileDownloader(ProgressReporter(), cmd)

        with mock.patch.object(downloader, 'check_capacity', return_value=(True, None)):
            with mock.patch.object(downloader, 'use_wget', return_value=8):
                success, error = downloader.download()

        self.assertFalse(success)
        self.assertNotIn('secret', error)
        self.assertIn('user:***@example.com', error)

    def test_file_download_atomically_replaces_destination(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            downloader = file_downloader.FileDownloader(
                ProgressReporter(),
                _file_download_cmd('atomic-file-download-api', source, destination)
            )

            def copy_to_temporary_path(_):
                shutil.copyfile(source, downloader.temporary_path)

            with mock.patch.object(downloader, 'check_capacity', return_value=(True, None)):
                with mock.patch.object(downloader.t_shell, 'call', side_effect=copy_to_temporary_path):
                    success, error = downloader.download()

            self.assertTrue(success, error)
            with open(destination, 'rb') as stream:
                self.assertEqual(b'new-content', stream.read())
            self.assertFalse(os.path.exists(downloader.temporary_path))

    def test_file_download_cancel_preserves_existing_destination(self):
        api_id = 'cancel-atomic-file-download-api'
        with tempfile.TemporaryDirectory() as directory:
            source = os.path.join(directory, 'source')
            destination = os.path.join(directory, 'destination')
            with open(source, 'wb') as stream:
            ...
            downloader = file_downloader.FileDownloader(
                ProgressReporter(),
                _file_download_cmd(api_id, source, destination)
            )

            def copy_then_cancel(_):
                shutil.copyfile(source, downloader.temporary_path)
                rsp = plugin.CancelJobResponse()
                with mock.patch('zstacklib.utils.plugin.traceable_shell.cancel_job', return_value=False):
                    plugin.cancel_job(_cancel_cmd(api_id, allow_task_not_found=False), rsp)
            ...
            with mock.patch.object(downloader, 'check_capacity', return_value=(True, None)):
                with mock.patch.object(downloader.t_shell, 'call', side_effect=copy_then_cancel):
                    success, error = downloader.download()

            self.assertFalse(success)
            self.assertEqual('download canceled before completion', error)
            with open(destination, 'rb') as stream:
                self.assertEqual(b'old-content', stream.read())
            self.assertFalse(os.path.exists(downloader.temporary_path))
    ...
    def test_linux_wget_rechecks_cancellation_after_file_size_probe(self):
        ...
        def shell_call(cmd, workdir=None, **kwargs):
